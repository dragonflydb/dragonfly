...
int zmalloc_get_allocator_fragmentation_step(float ratio, struct fragmentation_info* info) {
  ...
  if (info->bin == MI_BIN_FULL) {  // reached end of bins, reset state
    ...
    // as its tracked by zmalloc_heap->pages_full_size variable.
    info->committed += zmalloc_heap->pages_full_size;

    // TODO: it's a test code that makes sure `pages_full_size` is correct.
