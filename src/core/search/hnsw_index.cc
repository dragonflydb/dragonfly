...
//  private:
//   // Function requires that we hold mutex while resizing index. resizeIndex is not thread safe
//   with
...
struct HnswlibAdapter {
  ...
  explicit HnswlibAdapter(const SchemaField::VectorParams& params) : dim_(params.dim) {
    static auto hnsw_build_paramesters = R"(
    {
        "dtype": "float32",
        "metric_type": "$0",
        "dim": $1,
    ...
    const std::string similarity = params.sim == VectorSimilarity::L2 ? "l2" : "ip";

    index_ = vsag::Factory::CreateIndex(
                 "hnsw", absl::Substitute(hnsw_build_paramesters, similarity, params.dim,
