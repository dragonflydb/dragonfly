...
TEST_F(CompressedSortedSetTest, MergeBench) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    set_intersection(v1.begin(), v1.end(), l1.begin(), l1.end(), back_inserter(vout));
    VLOG(0) << (clock() - start);
  }
}

TEST_F(CompressedSortedSetTest, TreeBench) {
  absl::btree_set<int> v1;
  vector<int> v2;
  dfly::BPTree<int> v3;
  ...
  for (int i = 0; i < 10; i++) {
    vout.clear();
    auto start = clock();
    v3.Iterate(500'000, 1'000'000, [&vout](auto v) {
      vout.push_back(v);
      return true;
    });
    // for (auto it = v2.begin() + 500'000; it != v2.end(); it++)
    //   vout.push_back(*it);
    // set_intersection(l1.begin(), l1.end(), l2.begin(), l2.end(), back_inserter(vout));=
    VLOG(0) << (clock() - start);
  }
