...
bool PageCannotRealloc(mi_page_usage_stats_t s) {
  const bool page_full = s.flags & 1;
  const bool heap_mismatch = s.flags & 2;
  const bool malloc_page = s.flags & 4;
...
TEST_F(PageUsageStatsTest, Defrag) {
...

  {
    ...
    for (const auto& stat : p.Stats()) {
      EXPECT_TRUE(stat.should_realloc || PageCannotRealloc(stat));
    }
  }
