...
class Interpreter {
 ...
 private:
  ...
  // Pool interpreter stamped with its owner and generation.
  Interpreter(InterpreterManager* owner, uint64_t generation);
  ...
  // owner catches a foreign return, generation catches a stale (post-Reset) one.
