...
std::error_code JournalExecutor::Execute(journal::ParsedEntry::CmdData& cmd) {
  ...
  CommandContext cntx{conn_context_.transaction, &reply_builder_, &conn_context_};
  service_->DispatchCommand(span, &cntx);
  return cntx.is_oom ? make_error_code(errc::not_enough_memory) : error_code();
}
