...
size_t CmdSerializer::SerializeDelayedEntries(bool force,
  ...
  ready_entries.reserve(delayed_entries_.size());
  ...
  for (auto it = delayed_entries_.begin(); it != delayed_entries_.end();) {
    ...
    if (tiered_keys && !tiered_keys->contains(key)) {
      ++it;
      continue;
    }

    // Extract before blocking on the future so no delayed_entries_ iterator survives a yield.
    auto current = it++;
