...
void CmdMSetEx(CmdArgList args, CommandContext* cmnd_cntx) {
  ...
  bool nx = (sparams.flags & SetCmd::SET_IF_NOTEXIST);
  bool xx = (sparams.flags & SetCmd::SET_IF_EXISTS);
  ...
  AggregateStatus result;
  // Memory order relaxed is safe here because tx()->Execute() acts as a barrier.
  atomic_bool condition_met{true};

  // If we have NX or XX, we need to check preconditions atomically
  if (nx || xx) {
    auto check_cb = [&](Transaction* t, EngineShard* es) {
      if (!condition_met.load(memory_order_relaxed))
        return OpStatus::OK;

      auto shard_args = t->GetShardArgs(es->shard_id());
      auto op_args = t->GetOpArgs(es);
      // Args are interleaved key-value
      for (auto it = shard_args.begin(); it != shard_args.end(); ++it) {
        string_view key = *it;
        ++it;  // skip value

        auto res = op_args.GetDbSlice().FindReadOnly(t->GetDbContext(), key);
        bool exists = IsValid(res.it);

        if (nx && exists) {
          condition_met.store(false, memory_order_relaxed);
          return OpStatus::OK;
        }
    ...
    // First hop: check conditions
    cmnd_cntx->tx()->Execute(std::move(check_cb), false);
  }

  // Second hop (or only hop): execute MSET
  auto cb = [&](Transaction* t, EngineShard* shard) {
    if (!condition_met.load(memory_order_relaxed)) {
      return OpStatus::OK;
    }
    ...
    ShardArgs args = t->GetShardArgs(shard->shard_id());
    // We pass sparams with NX/XX constants but OpMSet/SetCmd handle them per key,
    // which is fine because we've already verified the global condition.
    // However, SetCmd with NX will fail if key exists (which we verified it doesn't).
    // SetCmd with XX will fail if key doesn't exist (verified it does).
    // So we can safely pass sparams as is.
  ...
  } else if (*result == OpStatus::KEY_EXISTS || *result == OpStatus::KEY_NOTFOUND ||
             *result == OpStatus::SKIPPED) {
    cmnd_cntx->SendLong(0);
  } else {
