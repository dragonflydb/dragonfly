...
OpResult<DbSlice::Iterator> FindKeyAndSetExpiry(const GetAndTouchParams& params) {
  ...
  auto it = db_slice.FindMutable(ctx, params.key, OBJ_STRING);
  if (!IsValid(it->it)) {
    return OpStatus::KEY_NOTFOUND;
  }
  ...
  auto update = db_slice.UpdateExpire(ctx, it->it, it->exp_it, params.expire_params);
  if (!update.ok()) {
...
void StringFamily::GAT(CmdArgList args, const CommandContext& cmnd_cntx) {
  ...
  auto ordered_by_shard = ReorderShardResults(mget_resp, cmnd_cntx.tx, args.size(), true);
