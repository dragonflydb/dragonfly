...
OpResult<DbSlice::ItAndUpdater> DbSlice::FindMutableInternal(const Context& cntx, string_view key,
  ...
  auto omit = IsOmittableWrite(cntx, ChangeReq{it.GetInnerIt()});
...
OpResult<DbSlice::ItAndUpdater> DbSlice::AddOrFindInternal(const Context& cntx, string_view key,
  ...
  if (!change_cb_.empty()) {
    for (bool consistent = false; !consistent;) {
      ...
      if (auto omit = IsOmittableWrite(cntx, bucket_set); omit.skip_serialize) {
        skip_serialize = true;
...
DbSlice::OmitDecision DbSlice::IsOmittableWrite(const Context& cntx, const ChangeReq& req) {
