...
#define BIND_CONFIG(var)         \
  [](const char* nm, auto val) { \
    var = val;                   \
    return true;                 \
  }

#define BIND_ENUM_CONFIG(enum_arr, dest_var)                                                     \
  [](const char* nm, const std::string& val) {                                                   \
    return ::dfly::ValidateConfigEnum(nm, val, enum_arr, ABSL_ARRAYSIZE(enum_arr), &(dest_var)); \
  }
...
#define CONFIG_string(name, val, txt, validator)                                                   \
  ...
  static ::dfly::DashStr<sizeof(#name)> _dash_##name(#name);                                       \
  static GFLAGS_NAMESPACE::FlagRegisterer o_##name(_dash_##name.str(), MAYBE_STRIPPED_HELP(txt),   \
                                                   __FILE__, FLAGS_no##name,                       \
