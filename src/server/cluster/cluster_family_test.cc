...
TEST_F(ClusterFamilyTest, DflyMigrateFlowInvalidShardId) {
  ...
  // Before DFLYMIGRATE INIT, ShardNum() is 0. Any flow should be rejected.
  ...
  EXPECT_THAT(Run({"DFLYMIGRATE", "FLOW", "id0", "9999"}), ErrArg("invalid shard id"));

  // Flow with unknown syncid should still return syncid not found
