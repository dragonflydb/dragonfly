...
TEST_F(StringFamilyTest, MSetEx) {
  ...
  // NX + XX combined should error
  EXPECT_THAT(Run({"msetex", "1", "k", "v", "NX", "XX"}), ErrArg("syntax"));

  // NX: Success (keys don't exist)
  EXPECT_EQ(1, CheckedInt({"msetex", "2", "k1", "v1", "k2", "v2", "NX", "EX", "100"}));
  EXPECT_EQ(Run({"get", "k1"}), "v1");

  // NX: Failure (key exists)
  EXPECT_EQ(0, CheckedInt({"msetex", "2", "k1", "v1", "k2", "v2", "NX", "EX", "100"}));

  // XX: Failure (keys don't exist)
  Run({"del", "e1", "e2"});
  EXPECT_EQ(0, CheckedInt({"msetex", "2", "e1", "v1", "e2", "v2", "XX", "EX", "100"}));

  // XX: Success (keys exist)
  Run({"set", "e1", "old"});
  Run({"set", "e2", "old"});
  EXPECT_EQ(1, CheckedInt({"msetex", "2", "e1", "new", "e2", "new", "XX", "EX", "100"}));
  EXPECT_EQ(Run({"get", "e1"}), "new");
}
