...
// Reads from all source shards, merges in the coordinator, writes to the dest shard.
void CmdMerge(CmdArgList args, CommandContext* cmd_cntx) {
  ...
  auto read_cb = [&](Transaction* t, EngineShard* shard) -> OpStatus {
    ...
    for (string_view key : src_keys) {
      ...
      cms_list.emplace_back(key, cms->width(), cms->depth(), cms->total_count(), cms->Data(),
                            counter_count);
    }
  ...
  if (all_cms_data.size() != src_keys.size()) {
    tx->Conclude();
    return rb->SendError(kCmsNotFound);
  }
