...
TEST_F(GenericFamilyTest, Dump) {
  ASSERT_THAT(RDB_VERSION, 11);
  uint8_t EXPECTED_STRING_DUMP[13] = {0x00, 0xc0, 0x13, 0x0b, 0x00, 0x9a, 0x3e,
                                      0x83, 0xf2, 0xc1, 0xef, 0xbf, 0x56};

  uint8_t EXPECTED_HASH_DUMP[] = {0x10, 0x0c, 0x0c, 0x00, 0x00, 0x00, 0x02, 0x00,
                                  0x13, 0x01, 0xc4, 0xd2, 0x02, 0xff, 0x0b, 0x00,
                                  0xd1, 0x60, 0x9f, 0x1b, 0xa6, 0x3A, 0xc5, 0xff};

  uint8_t EXPECTED_LIST_DUMP[] = {0x12, 0x01, 0x02, 0x09, 0x09, 0x00, 0x00, 0x00,
                                  0x01, 0x00, 0x14, 0x01, 0xff, 0x0b, 0x00, 0x42,
                                  0x90, 0xda, 0x47, 0x1d, 0x6D, 0xaf, 0x03};
...
TEST_F(GenericFamilyTest, Restore) {
  ...
  // The following dump was taken from Redis 7 with rdb version 11
  // by running `set key 1234` && `dump key`
  uint8_t STRING_DUMP_REDIS[] = {0x00, 0xc1, 0xd2, 0x04, 0x0b, 0x00, 0x69,
                                 0x58, 0xb5, 0xd2, 0xb9, 0x1d, 0xb5, 0x64};
