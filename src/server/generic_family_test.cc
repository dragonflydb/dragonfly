...
TEST_F(GenericFamilyTest, ScanWithAttr) {
  ...
  // before run get "foo", scan with a attr should return empty set
  resp = Run({"scan", "0", "attr", "a"});
  vec = StrArray(resp.GetVec()[1]);
  EXPECT_EQ(0, vec.size());

  // before run get "foo", scan with a attr should return "foo" and "hello"
  ...
  // after run get "foo", scan with a attr should return "foo"
  resp = Run({"scan", "0", "attr", "a"});
  vec = StrArray(resp.GetVec()[1]);
  EXPECT_EQ(1, vec.size());
  EXPECT_EQ(vec[0], "foo");

  // after run get "foo", scan with a attr should return "hello"
  resp = Run({"scan", "0", "attr", "u"});
  vec = StrArray(resp.GetVec()[1]);
  EXPECT_EQ(1, vec.size());
  EXPECT_EQ(vec[0], "hello");
