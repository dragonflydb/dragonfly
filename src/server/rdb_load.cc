...
error_code RdbLoader::Load(io::Source* src) {
  ...
  while (!stop_early_.load(memory_order_relaxed)) {
    ...
    if (type == RDB_OPCODE_SEGMENT_COUNT) {
      uint64_t prime_count, expires_count;
      SET_OR_RETURN(LoadLen(nullptr), prime_count);
      SET_OR_RETURN(LoadLen(nullptr), expires_count);
      uint32_t load_shard_id =
          EngineShard::tlocal() ? EngineShard::tlocal()->shard_id() : UINT32_MAX;

      VLOG(1) << "prime_count=" << prime_count << ", expires_count=" << expires_count << " "
              << shard_id_ << " " << load_shard_id << " " << shard_count_;
