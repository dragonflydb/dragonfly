...
bool SliceSnapshot::PushSerialized(bool force) {
  ...
  if (!delayed_entries_.empty()) {
    ...
    do {
      ...
      PrimeValue pv{entry.value.Get()};  // Migh block until the future resolves.

      // TODO: to introduce RdbSerializer::SaveString that can accept a string value directly.
      serializer_->SaveEntry(entry.key, pv, entry.expire, entry.dbid,
                             entry.mc_flags);
...
void SliceSnapshot::SerializeExternal(DbIndex db_index, PrimeKey key,
                                      const PrimeValue& pv, time_t expire_time, uint32_t mc_flags) {
  ...
  delayed_entries_.push_back(
      {db_index, std::move(key), std::move(future), expire_time, mc_flags});
