...
MemoryScope::MemoryScope(int obj_type, const DbSlice* db_slice)
  ...
  DCHECK(parent_ == nullptr || db_slice == nullptr) << "child scope inherits db slice from parent";
  tl_top_scope = this;
}

MemoryScope::~MemoryScope() {
  DCHECK_EQ(tl_top_scope, this);
...
TxMemoryScope::TxMemoryScope(int obj_type, const DbSlice* db_slice)
    : core_(obj_type, db_slice), prev_hook_(ThisFiber::SetSwitchHook({MemTrackerHook})) {
...
TxMemoryScope::~TxMemoryScope() {
  // Check that if suspended via the fiber hook then resume was also called.
  DCHECK(!suspended_);
