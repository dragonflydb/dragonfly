...
ABSL_FLAG(size_t, filtered_knn_search_threshold, 8192,
          "If prefilter results are below this threshold, we will do filtered exact subset search "
...
vector<SearchResult> SearchGlobalHnswIndex(
  ...
  if (prefilter_global_docs_ids) {
    ...
    if (prefilter_global_docs_ids->size() < absl::GetFlag(FLAGS_filtered_knn_search_threshold)) {
      knn_results =
          index->FilteredKnn(knn->vec.first.get(), knn->limit, *prefilter_global_docs_ids);
    } else {
      knn_results =
