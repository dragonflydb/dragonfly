...
class RdbTest : public BaseFamilyTest {
 protected:
  ...
  std::error_code LoadRdbBytes(const uint8_t* data, size_t size) {
    return pp_->at(0)->Await([&] {
      io::BytesSource src(string_view(reinterpret_cast<const char*>(data), size));
      RdbLoadContext load_context;
      RdbLoader loader(service_.get(), &load_context);
  ...
  std::error_code LoadRdbBounded(const std::string& rdb) {
    return pp_->at(0)->Await([&] {
...
// Odd hash listpack (f1, v1, f2) on the file-load path must fail the load, else HGETALL
// NULL-derefs the unpaired value.
TEST_F(RdbTest, LoadRejectsOddHashListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00, 0x10,
                   0x03, 0x70, 0x77, 0x6e, 0x13, 0x13, 0x00, 0x00, 0x00, 0x03, 0x00, 0x82,
                   0x66, 0x31, 0x03, 0x82, 0x76, 0x31, 0x03, 0x82, 0x66, 0x32, 0x03, 0xff,
                   0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_TRUE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"exists", "pwn"}), IntArg(0));
  Run({"hgetall", "pwn"});  // must not crash
}

// Odd zset listpack (m1, 1, m2): a member without a score NULL-derefs on ZRANK.
TEST_F(RdbTest, LoadRejectsOddZsetListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00,
                   0x11, 0x03, 0x70, 0x77, 0x6e, 0x12, 0x12, 0x00, 0x00, 0x00, 0x03,
                   0x00, 0x82, 0x6d, 0x31, 0x03, 0x81, 0x31, 0x02, 0x82, 0x6d, 0x32,
                   0x03, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_TRUE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"exists", "pwn"}), IntArg(0));
  Run({"zrank", "pwn", "zzz"});  // must not crash
}

// Header count (2) disagreeing with the body (3 entries) passes shallow validation; the deep
// check must reject it.
TEST_F(RdbTest, LoadRejectsMismatchedHashListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00, 0x10,
                   0x03, 0x70, 0x77, 0x6e, 0x13, 0x13, 0x00, 0x00, 0x00, 0x02, 0x00, 0x82,
                   0x66, 0x31, 0x03, 0x82, 0x76, 0x31, 0x03, 0x82, 0x66, 0x32, 0x03, 0xff,
                   0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_TRUE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"exists", "pwn"}), IntArg(0));
...
// The unknown-count sentinel (numele == 0xffff) is odd, but a valid even body must still load.
TEST_F(RdbTest, LoadAcceptsSentinelHashListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00,
                   0x10, 0x03, 0x70, 0x77, 0x6e, 0x0f, 0x0f, 0x00, 0x00, 0x00, 0xff,
                   0xff, 0x82, 0x66, 0x31, 0x03, 0x82, 0x76, 0x31, 0x03, 0xff, 0xff,
                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_FALSE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"hgetall", "pwn"}).GetVec(), UnorderedElementsAre("f1", "v1"));
}

// The sentinel is not a bypass: an odd body (3 entries) under numele == 0xffff is still rejected.
TEST_F(RdbTest, LoadRejectsSentinelOddHashListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00, 0x10,
                   0x03, 0x70, 0x77, 0x6e, 0x13, 0x13, 0x00, 0x00, 0x00, 0xff, 0xff, 0x82,
                   0x66, 0x31, 0x03, 0x82, 0x76, 0x31, 0x03, 0x82, 0x66, 0x32, 0x03, 0xff,
                   0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_TRUE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"exists", "pwn"}), IntArg(0));
...
// The zset load path has its own validator; header count (2) disagreeing with the body (3
// entries: m1, 1, m2) must be rejected by the deep check.
TEST_F(RdbTest, LoadRejectsMismatchedZsetListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00,
                   0x11, 0x03, 0x70, 0x77, 0x6e, 0x12, 0x12, 0x00, 0x00, 0x00, 0x02,
                   0x00, 0x82, 0x6d, 0x31, 0x03, 0x81, 0x31, 0x02, 0x82, 0x6d, 0x32,
                   0x03, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_TRUE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"exists", "pwn"}), IntArg(0));
...
// A valid even-paired zset {m1:1} under the 0xffff sentinel must load intact.
TEST_F(RdbTest, LoadAcceptsSentinelZsetListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe,
                   0x00, 0x11, 0x03, 0x70, 0x77, 0x6e, 0x0e, 0x0e, 0x00, 0x00,
                   0x00, 0xff, 0xff, 0x82, 0x6d, 0x31, 0x03, 0x81, 0x31, 0x02,
                   0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_FALSE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_EQ(Run({"zscore", "pwn", "m1"}), "1");
}

// The sentinel is not a zset bypass: an odd body (m1, 1, m2) under numele == 0xffff is rejected.
TEST_F(RdbTest, LoadRejectsSentinelOddZsetListpack) {
  uint8_t rdb[] = {0x52, 0x45, 0x44, 0x49, 0x53, 0x30, 0x30, 0x31, 0x31, 0xfe, 0x00,
                   0x11, 0x03, 0x70, 0x77, 0x6e, 0x12, 0x12, 0x00, 0x00, 0x00, 0xff,
                   0xff, 0x82, 0x6d, 0x31, 0x03, 0x81, 0x31, 0x02, 0x82, 0x6d, 0x32,
                   0x03, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  EXPECT_TRUE(LoadRdbBytes(rdb, sizeof(rdb)));
  EXPECT_THAT(Run({"exists", "pwn"}), IntArg(0));
  Run({"zrank", "pwn", "zzz"});  // must not crash
...
TEST_F(RdbTest, LoadRejectsHugeDeclaredLength) {
  ...
  auto cleanup = absl::MakeCleanup([&] { unlink(rdb_file.c_str()); });
...
TEST_F(RdbTest, LoadRejectsTruncatedFile) {
  string body;
  body.push_back(static_cast<char>(RDB_OPCODE_SELECTDB));
  AppendLen(&body, 0);
  string rdb = WrapInRdb(body);
  rdb.resize(rdb.size() - 9);

  EXPECT_EQ(LoadRdbBounded(rdb), RdbError(rdb::errc::rdb_file_corrupted));
}
...
void AppendEmptyStreamValue(std::string* body) {
  AppendLen(body, 0);
  AppendLen(body, 0);
  AppendLen(body, 0);
  AppendLen(body, 0);
...
void AppendInt64(std::string* out, int64_t val) {
  uint8_t buf[8];
  absl::little_endian::Store64(buf, static_cast<uint64_t>(val));
