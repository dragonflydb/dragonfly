...
void ExtractIds(std::string_view wire_data, std::vector<uint32_t>& ids) {
  auto ReadLen = [](std::string_view* input) -> uint64_t {
    CHECK(!input->empty());

    PackedUIntMeta meta{static_cast<uint8_t>((*input)[0])};
    CHECK_GE(input->size(), 1u + meta.ByteSize());

    io::Bytes tail{reinterpret_cast<const uint8_t*>(input->data() + 1), meta.ByteSize()};
    auto res = ReadPackedUInt(meta, tail);
    CHECK(res.has_value());

    input->remove_prefix(1 + meta.ByteSize());
    return res.value();
  };

  while (!wire_data.empty()) {
    const uint8_t byte = static_cast<uint8_t>(wire_data[0]);
    if (byte == RDB_OPCODE_TAGGED_CHUNK) {
      CHECK_GE(wire_data.size(), 9u);

      uint32_t stream_id = absl::little_endian::Load32(wire_data.data() + 1);
      uint32_t payload_len = absl::little_endian::Load32(wire_data.data() + 5);

      ids.push_back(stream_id);

      ASSERT_GE(wire_data.size(), 9 + payload_len);
      wire_data.remove_prefix(9 + payload_len);
      continue;
    }

    if (byte == RDB_OPCODE_JOURNAL_BLOB) {
      ids.push_back(0);
      wire_data.remove_prefix(1);

      uint64_t num_entries = ReadLen(&wire_data);
      CHECK_EQ(num_entries, 1u);

      uint64_t blob_len = ReadLen(&wire_data);
      CHECK_GE(wire_data.size(), blob_len);

      wire_data.remove_prefix(blob_len);
      continue;
    }

    if (byte == RDB_OPCODE_FULLSYNC_END) {
      ids.push_back(0);
      wire_data.remove_prefix(9);
      continue;
    }

    FAIL() << "unexpected opcode " << static_cast<unsigned>(byte);
  }
}

TEST_F(RdbTest, TwoKeyChunksWireFormat) {
  // large hset will break in chunks because it will promote to strmap on growing large enough
  for (int i = 0; i < 500; ++i) {
    auto resp = Run({"hset", "A", StrCat("field:", i), string{100, 'x'}});
    ASSERT_THAT(resp, IntArg(1));
  }

  vector<string> output;
  bool added_small = false;

  auto test = [&] {
    DbContext ctx{&namespaces->GetDefaultNamespace(), 0, GetCurrentTimeMs()};
    auto& slice = ctx.GetDbSlice(0);
    auto big_val = slice.FindReadOnly(ctx, "A", OBJ_HASH);
    ASSERT_TRUE(big_val.ok());

    PrimeKey big_key{"A"};
    const PrimeValue& big_value = big_val.value()->second;

    PrimeKey small_key{"B"};
    PrimeValue small_value{"small"};
    ...
    RdbSerializer serializer{
        ...
        [&](string blob) {
          ...
          if (!added_small) {
            // add only once
            added_small = true;
            // add a small entry mid-stream of big entry
            serializer.SetCurrentStreamId(2);
            ASSERT_TRUE(serializer.SaveEntry(small_key, small_value, 0, 0, 0).has_value());
          }
        },
        256};

    serializer.SetTagEntries(true);
    serializer.SetCurrentStreamId(1);
    ASSERT_TRUE(serializer.SaveEntry(big_key, big_value, 0, 0, 0).has_value());

    output.push_back(serializer.Flush(RdbSerializerBase::FlushState::kFlushEndEntry));
  };
  pp_->at(0)->Await(test);

  // The big key broken at least once
  ASSERT_GE(output.size(), 3);
  ASSERT_TRUE(added_small);

  std::vector<uint32_t> ids;
  const auto wire_data = absl::StrJoin(output, "");
  ExtractIds(wire_data, ids);
  // first three items are guaranteed because of mid-stream write
  ASSERT_EQ(ids[0], 1u);
  ASSERT_EQ(ids[1], 2u);
  ASSERT_EQ(ids[2], 1u);
  // there may be more depending on flush count
  EXPECT_TRUE(std::all_of(ids.begin() + 3, ids.end(), [](auto id) { return id == 1u; }));
}

TEST_F(RdbTest, KeyMixedWithOpCodeWireFormat) {
  for (int i = 0; i < 500; ++i) {
    auto resp = Run({"hset", "A", StrCat("field:", i), string{100, 'x'}});
    ASSERT_THAT(resp, IntArg(1));
  }
    ...
    RdbSerializer serializer{CompressionMode::NONE,
                             [&](string blob) {
                               ...
                               if (!added_journal) {
                                 // add only once
                                 added_journal = true;
                                 ASSERT_FALSE(serializer.WriteJournalEntry("foobar"));
                                 ASSERT_FALSE(serializer.WriteJournalEntry("foobar"));
                                 ASSERT_FALSE(serializer.SendFullSyncCut());
                               }
                             },
                             256};
  ...
  ASSERT_TRUE(added_journal);

  std::vector<uint32_t> ids;
  const auto wire_data = absl::StrJoin(output, "");
  ExtractIds(wire_data, ids);
  ASSERT_EQ(ids[0], 1u);
  ASSERT_EQ(ids[1], 0u);  // journal
  ASSERT_EQ(ids[2], 0u);  // journal
  ASSERT_EQ(ids[3], 0u);  // full sync cut
  ASSERT_EQ(ids[4], 1u);
  EXPECT_TRUE(std::all_of(ids.begin() + 5, ids.end(), [](auto id) { return id == 1u; }));
}
