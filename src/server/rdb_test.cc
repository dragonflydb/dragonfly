...
TEST_F(RdbTest, SnapshotEgressThrottle) {
  ...
  // Throttle to a small fraction of the measured capacity so the limit - not CPU - is the
  // bottleneck on any machine (including slow ASAN/CI). Aim for a run of at least a couple
  // of seconds so several sliding windows elapse. Pacing makes the achieved average rate
  // converge to the limit, so the expected duration is ~bytes/limit == target_sec.
  double target_sec = std::max(2.0, t_base * 8);
  uint64_t limit = uint64_t(bytes / target_sec);
  ...
  LOG(INFO) << "egress throttle: bytes=" << bytes2 << " limit=" << limit << "B/s t_base=" << t_base
            << "s t_lim=" << t_lim << "s rate=" << uint64_t(rate) << "B/s";

  // Core guarantee: the achieved egress rate stays at/below the limit. The only slack is the
  // one-window initial burst plus per-bucket overshoot, comfortably within 1.6x.
  EXPECT_LE(rate, limit * 1.6) << "egress exceeded the configured limit";
