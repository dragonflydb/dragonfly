...
async def test_expiry_on_replica(df_factory: DflyInstanceFactory):
    """
    Test that expired keys on replica return nil/(-2) for GET/TTL
    without waiting for master to trigger lazy expiry first.
    Reproduces a bug where ExpireIfNeeded returned valid iterators
    for expired keys on replicas, causing GET to return stale values
    and TTL to return huge unsigned values (~UINT64_MAX).
    """
