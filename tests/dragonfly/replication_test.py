...
@dfly_args({"proactor_threads": 4, "default_lua_flags": "allow-undeclared-keys"})
async def test_eval_expire_replication(df_factory: DflyInstanceFactory):
    """Verify that lazy expiry inside global EVAL maintains data parity with replica.

    Regression test: ExpireIfNeeded was completely disabled during global
    transactions (EVAL with allow-undeclared-keys) because the
    shard_lock()->Check(EXCLUSIVE) guard blocked it. With the fix, lazy expiry
    works inside global EVAL and the deletion is journaled to replicas.
    """
    master = df_factory.create()
    replica = df_factory.create()
    ...
    # Set keys with 1ms TTL via EVAL. Each EVAL holds the global shard lock
    # during execution, so heartbeat cannot expire the key between SET and PEXPIRE.
    set_script = "redis.call('SET', KEYS[1], ARGV[1]); redis.call('PEXPIRE', KEYS[1], 1)"
    pipe = c_master.pipeline(transaction=False)
    for i in range(n_keys):
        pipe.eval(set_script, 1, f"expkey:{i}", f"val:{i}")
    for i in range(n_keys):
        pipe.set(f"permkey:{i}", f"pval:{i}")
    await pipe.execute()

    # By now the 1ms TTL has passed. Trigger lazy expiry via global EVAL reads.
    # With the bug, the shard lock blocks lazy expiry and EVAL returns stale values.
    # Active expiry may race and delete some keys, so we tolerate that — the key
    # assertion is that NO key returns a stale value.
    get_script = "return redis.call('GET', KEYS[1])"
    stale_keys = []
    for i in range(n_keys):
        result = await c_master.eval(get_script, 1, f"expkey:{i}")
        if result is not None:
            stale_keys.append(f"expkey:{i}")

    assert len(stale_keys) == 0, (
        f"{len(stale_keys)} keys returned stale values from EVAL instead of nil: "
        f"{stale_keys[:10]}..."
    )

    # Permanent keys should still be alive on master.
    for i in range(n_keys):
        result = await c_master.get(f"permkey:{i}")
        assert result is not None, f"permkey:{i} should still exist on master"

    await check_all_replicas_finished([c_replica], c_master)

    # Verify data parity: expired keys gone on replica too.
    for i in range(n_keys):
        result = await c_replica.get(f"expkey:{i}")
        assert result is None, f"expkey:{i} should have expired on replica"

    # Permanent keys present on replica.
    for i in range(n_keys):
        result = await c_replica.get(f"permkey:{i}")
        assert result is not None, f"permkey:{i} should exist on replica"
