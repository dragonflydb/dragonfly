...
# Environment variables recognized as user-facing benchmark configuration. Snapshot
# only values explicitly present at invocation so the provenance log does not claim
# defaults as user choices or include unrelated shell environment.
USER_COMMAND_ENV_NAMES=(
    ADAPTIVE_AVG ADAPTIVE_AVG_MIN_RUNS ADAPTIVE_AVG_MAX_RUNS
    BATCH_MODE_RUN_ONLY BATCH_RERUN_ON_VARIANCE BENCH_LOG_DIR METRICS_DIR
...
declare -A USER_COMMAND_ENV
for user_command_env_name in "${USER_COMMAND_ENV_NAMES[@]}"; do
    if [[ -v "$user_command_env_name" ]]; then
        USER_COMMAND_ENV["$user_command_env_name"]="${!user_command_env_name}"
    fi
done
...
# different noise, so they are judged separately. The goal per cell is a TRUSTED GROUP: >=
# ADAPTIVE_AVG_MIN_RUNS samples whose RPS all fall within MAX_RESULT_VARIANCE_PCT of each other.
#
# Flow: run the full matrix ADAPTIVE_AVG_MIN_RUNS times (baseline), then RE-RUN ONLY the cells that
# still lack a trusted group - one extra sample per round, reusing all prior samples - until every
# cell has a trusted group or reaches ADAPTIVE_AVG_MAX_RUNS samples. Cells that already agree are
# NEVER re-run, which is where the time is saved. The final report averages each cell's tightest
# trusted group and lists the dropped (untrusted) samples.
#   * ADAPTIVE_AVG_MIN_RUNS   minimum samples per cell AND trusted-group size (always >= 3).
#   * ADAPTIVE_AVG_MAX_RUNS   per-cell hard cap on samples (stop retrying a cell that never settles).
...
# ---------------------------------------------------------------------------
ADAPTIVE_AVG_MIN_RUNS="${ADAPTIVE_AVG_MIN_RUNS:-3}"
ADAPTIVE_AVG_MAX_RUNS="${ADAPTIVE_AVG_MAX_RUNS:-9}"
...
Batch mode (--batch <file> | -b <file>):
  ...
  Still honored from the environment with --batch (they are global, not per-run):
    ...
    BATCH_MODE_RUN_ONLY                 - comma-separated 1-based run indices or inclusive
                                          ranges to run, e.g. BATCH_MODE_RUN_ONLY=1-3,7,9.
                                          A range's end must be >= its start. Any index outside
                                          1..N aborts immediately before any run starts.
  ...
  Logs survive every run:
    ...
    final summary reports failures and the script exits non-zero if any run failed.
        The main batch log begins with the original command and only this script's explicitly
        supplied configuration variables (unrelated shell environment is not recorded).
...
Environment variables (for cross-machine benchmarking):
  ...
  SERVER_MAXMEMORY: Safety-net memory cap passed to the server as --maxmemory.
    ...
    SERVER_PIN:    Remote mode only. 'on' (default) enables server CPU pinning; 'off' disables it.
                                                                This controls whether taskset is used. With pinning enabled and no
                                                                SERVER_CPUS, remote mode chooses the server CPU list automatically.
                                                                Local mode always pins the server.
    SERVER_CPUS:  Explicit CPU list for the server taskset (e.g. SERVER_CPUS=1-3 or 0,2).
                                                                This controls which CPUs the server uses. In local mode it requires
                                                                CLIENT_CPUS as well; in remote mode it overrides automatic selection.
  ...
  TRACY_CAPTURE_SECONDS: Optional positive duration for each Tracy capture. The benchmark keeps
                running for BENCH_DURATION; Tracy records only this initial post-prefill window.
                Example: TRACY_CAPTURE=1 TRACY_CAPTURE_SECONDS=3
...
run_batch_mode() {
    ...
    # --- BATCH_MODE_RUN_ONLY: optional subset of run indices/ranges (1-based) --
    # e.g. BATCH_MODE_RUN_ONLY=1-3,7,9. Ranges are inclusive and normalized to
    # individual indices before execution. Invalid or out-of-range values abort
    # before any run starts.
    local selected_runs_csv=""
    if [[ -n "${BATCH_MODE_RUN_ONLY:-}" ]]; then
        local _ro _clean="${BATCH_MODE_RUN_ONLY//[[:space:]]/}"
        local _range_start _range_end _range_index
        local -a _ro_list selected_runs=()
        IFS=',' read -ra _ro_list <<< "$_clean"
        for _ro in "${_ro_list[@]}"; do
            if [[ "$_ro" =~ ^([0-9]+)-([0-9]+)$ ]]; then
                _range_start=${BASH_REMATCH[1]}
                _range_end=${BASH_REMATCH[2]}
                if (( _range_start < 1 || _range_end < _range_start ||
                      _range_end > ${#run_lines[@]} )); then
                    echo "[!] Error: BATCH_MODE_RUN_ONLY has invalid run range '${_ro}'."
                    echo "    Ranges must be within 1..${#run_lines[@]} with end >= start."
                    exit 1
                fi
                for (( _range_index = _range_start; _range_index <= _range_end; ++_range_index )); do
                    selected_runs+=("$_range_index")
                done
            elif [[ "$_ro" =~ ^[0-9]+$ ]] && (( _ro >= 1 && _ro <= ${#run_lines[@]} )); then
                selected_runs+=("$_ro")
            else
        ...
        selected_runs_csv=$(IFS=,; echo "${selected_runs[*]}")
        echo "[*] BATCH_MODE_RUN_ONLY active: running only run(s) ${selected_runs_csv} of ${#run_lines[@]}."
    fi
    ...
    local self="$0"
    local batch_log="${outdir}/batch_run_main_${ts}.log"
    local failures=0 rc
    local script_command user_command user_command_env_name user_command_env_value
    user_command=""
    for user_command_env_name in "${USER_COMMAND_ENV_NAMES[@]}"; do
        [[ -v "USER_COMMAND_ENV[$user_command_env_name]" ]] || continue
        printf -v user_command_env_value '%q' "${USER_COMMAND_ENV[$user_command_env_name]}"
        user_command+="${user_command_env_name}=${user_command_env_value} "
    done
    printf -v script_command '%q ' "$0" --batch "$desc_file"
    user_command+="$script_command"
    user_command="${user_command% }"
    set +e
    {
        echo "User command to run this script:"
        echo "${user_command}"
        echo "========================================"
        ...
        for line in "${run_lines[@]}"; do
            ...
            # Honor BATCH_MODE_RUN_ONLY: skip runs not in the selected set.
                if [[ -n "$selected_runs_csv" ]] && [[ ",${selected_runs_csv}," != *",${idx},"* ]]; then
                echo ""
...
# baseline full-matrix passes; per-cell targeted retries then top up failing cells up to
# ADAPTIVE_AVG_MAX_RUNS.
if ! [[ "$ADAPTIVE_AVG_MIN_RUNS" =~ ^[0-9]+$ ]] || [[ "$ADAPTIVE_AVG_MIN_RUNS" -lt 1 ]]; then
    echo "[!] Error: ADAPTIVE_AVG_MIN_RUNS must be a positive integer. Got: '$ADAPTIVE_AVG_MIN_RUNS'"; exit 1
fi
if ! [[ "$ADAPTIVE_AVG_MAX_RUNS" =~ ^[0-9]+$ ]] ||
   [[ "$ADAPTIVE_AVG_MAX_RUNS" -lt "$ADAPTIVE_AVG_MIN_RUNS" ]]; then
    echo "[!] Error: ADAPTIVE_AVG_MAX_RUNS must be an integer >= ADAPTIVE_AVG_MIN_RUNS (${ADAPTIVE_AVG_MIN_RUNS}). Got: '$ADAPTIVE_AVG_MAX_RUNS'"; exit 1
fi
if [[ "$RUNS" -gt 1 && "$ADAPTIVE_AVG" == "1" ]]; then
    ADAPTIVE=1
    [[ "$RUNS" -lt "$ADAPTIVE_AVG_MIN_RUNS" ]] && RUNS=$ADAPTIVE_AVG_MIN_RUNS
    [[ "$RUNS" -gt "$ADAPTIVE_AVG_MAX_RUNS" ]] && ADAPTIVE_AVG_MAX_RUNS=$RUNS
else
    ADAPTIVE=0
fi
...
TRACY_CAPTURE_SECONDS=${TRACY_CAPTURE_SECONDS:-""}
...
if [[ "$TRACY_CAPTURE" == "1" ]]; then
    ...
    if [[ -n "$TRACY_CAPTURE_SECONDS" && ! "$TRACY_CAPTURE_SECONDS" =~ ^[1-9][0-9]*$ ]]; then
...
start_tracy_capture() {
    ...
    local -a capture_args=(-f -o "$TRACY_CAPTURE_PATH")
    if [[ -n "$TRACY_CAPTURE_SECONDS" ]]; then
        capture_args+=(-s "$TRACY_CAPTURE_SECONDS")
        echo "  [tracy] Capturing ${TRACY_CAPTURE_SECONDS}s to ${TRACY_CAPTURE_PATH}"
    else
        echo "  [tracy] Capturing to ${TRACY_CAPTURE_PATH}"
    fi
    tracy-capture "${capture_args[@]}" > "${TRACY_CAPTURE_PATH}.log" 2>&1 &
    TRACY_CAPTURE_PID=$!
    sleep 0.5
...
# still need another sample: NOT trusted AND fewer than ADAPTIVE_AVG_MAX_RUNS attempts so far. A
# cell is TRUSTED when some subset of >= ADAPTIVE_AVG_MIN_RUNS positive RPS samples all fall within
# MAX_RESULT_VARIANCE_PCT of each other (the same tightest-window rule the final report averages).
# When CELL_RETRY ends up empty, every cell is either trusted or capped, and the retry loop stops.
_recompute_cell_status() {
    ...
    while IFS='|' read -r lbl mod pip cnt trusted; do
        [[ "$trusted" == "1" ]] && continue
        (( cnt >= ADAPTIVE_AVG_MAX_RUNS )) && continue
        CELL_RETRY["${lbl}|${mod}|${pip}"]=1
    done < <(awk -F'|' -v thr="$MAX_RESULT_VARIANCE_PCT" -v minn="$ADAPTIVE_AVG_MIN_RUNS" '
    {
...
# cell it determines the tightest trusted group (>= ADAPTIVE_AVG_MIN_RUNS positive samples within
...
#   * flags any cell that NEVER formed a trusted group even at the ADAPTIVE_AVG_MAX_RUNS cap.
report_untrusted_samples() {
    [[ $RUNS -le 1 ]] && return 0
    [[ -s "$ACCUM_FILE" ]] || return 0
    awk -F'|' -v thr="$MAX_RESULT_VARIANCE_PCT" -v minn="$ADAPTIVE_AVG_MIN_RUNS" '
    {
        key = $1 SUBSEP $2 SUBSEP $3
        if (!(key in seen)) { seen[key]=1; order[++n]=key; lbl[key]=$1; mod[key]=$2; pip[key]=$3; kc[key]=0 }
        i = ++kc[key]; rps[key,i] = $4 + 0
    ...
    multi_conn|single_conn|pubsub|conn|all)
        {
            ...
            # (>= ADAPTIVE_AVG_MIN_RUNS samples within MAX_RESULT_VARIANCE_PCT), one extra sample
            # per round, reusing all prior samples, until every cell is trusted or hits
            # ADAPTIVE_AVG_MAX_RUNS samples.
            ...
            if (( ADAPTIVE )); then
                ...
                while (( ${#CELL_RETRY[@]} > 0 )); do
                    r=$((r + 1))
                    echo ""
                    echo "############## Pass $r (retry: ${#CELL_RETRY[@]} untrusted cell(s), cap=${ADAPTIVE_AVG_MAX_RUNS}) ##############"
                    for _ds in "${DATA_SIZES[@]}"; do
                        DATA_SIZE="$_ds"
                        if [[ ${#DATA_SIZES[@]} -gt 1 ]]; then
                            DATA_SIZE_SUFFIX="_d${_ds}"
...
# flags any cell that never formed a trusted group (>= ADAPTIVE_AVG_MIN_RUNS within
# MAX_RESULT_VARIANCE_PCT) even at the ADAPTIVE_AVG_MAX_RUNS cap. This NEVER stops or fails the
# run - it is purely informational.
