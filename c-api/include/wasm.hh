...
static_assert(sizeof(intptr_t) == sizeof(int32_t) || sizeof(intptr_t) == sizeof(int64_t),
...
template <class T> class vec {
 ...
 public:
  ...
  static auto make(std::string s) -> vec<char> {
    auto v = vec(s.length());
    if (v)
      std::strncpy(v.get(), s.data(), s.length());
    return v;
  }
...
struct Limits {
  ...
  Limits(uint32_t min, uint32_t max = std::numeric_limits<uint32_t>::max()) : min(min), max(max) {
  }
};
...
enum class ExternKind : uint8_t { FUNC, GLOBAL, TABLE, MEMORY };
...
class WASM_API_EXTERN FuncType : public ExternType {
 ...
 public:
  static auto make(ownvec<ValType>&& params = ownvec<ValType>::make(),
                   ownvec<ValType>&& results = ownvec<ValType>::make()) -> own<FuncType>;
...
class Val {
 ...
 public:
  Val() : kind_(ValKind::EXTERNREF) {
    impl_.ref = nullptr;
  }
  explicit Val(int32_t i) : kind_(ValKind::I32) {
    impl_.i32 = i;
  }
  explicit Val(int64_t i) : kind_(ValKind::I64) {
    impl_.i64 = i;
  }
  explicit Val(float32_t z) : kind_(ValKind::F32) {
    impl_.f32 = z;
  }
  explicit Val(float64_t z) : kind_(ValKind::F64) {
    impl_.f64 = z;
  }
  explicit Val(own<Ref>&& r) : kind_(ValKind::EXTERNREF) {
    impl_.ref = r.release();
  }
  ...
  auto i32() const -> int32_t {
    assert(kind_ == ValKind::I32);
    return impl_.i32;
  }
  auto i64() const -> int64_t {
    assert(kind_ == ValKind::I64);
    return impl_.i64;
  }
  auto f32() const -> float32_t {
    assert(kind_ == ValKind::F32);
    return impl_.f32;
  }
  auto f64() const -> float64_t {
    assert(kind_ == ValKind::F64);
    return impl_.f64;
  }
  auto ref() const -> Ref* {
    assert(is_ref());
...
class WASM_API_EXTERN Func : public Extern {
 ...
 public:
  ...
  static auto make(Store*, const FuncType*, callback_with_env, void*,
...
class WASM_API_EXTERN Instance : public Ref {
 ...
 public:
  static auto make(Store*, const Module*, const vec<Extern*>&, own<Trap>* = nullptr)
      -> own<Instance>;
