...
4. RdbSave also creates a fiber (SaveBody) that pull all the blobs from the channel. Blobs might come in unspecified order though it's guaranteed that each blob is self sufficient but itself.
