...
CACHE_PREFIX = "test-results/ci-dashboard-cache"
REPORT_SOURCES = (
    ("junit/cpp", "*/ctest/*.xml"),
...
def s3_day_summaries(
    ...
    namespace = f"{CACHE_PREFIX}/{pipeline}"
        ...
        with tempfile.TemporaryDirectory(prefix=f"{day}-", dir=work_dir) as temporary:
            ...
            if summary is not None:
                ...
                if source_fingerprint(s3.list_reports(day)) == fingerprint:
                    ...
                    dashboard.write_day_cache(cache_path, {**header, "summary": summary})
...
def sync_dashboard(
    ...
    dashboard.build_from_summaries(
        s3_day_summaries(s3, inventories, work_dir, started),
        set(inventories),
...
def main() -> int:
    ...
    sync_dashboard(S3(args.source_bucket), output, work_dir, args.lookback_days)
    return 0
